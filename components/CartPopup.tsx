...
const CartPopup = () => {
  ...
  const getDisplayText = () => {
    ...
    const uniqueProducts = [...new Set(items.map(item => item.product?.name).filter(Boolean))];
    
    if (uniqueProducts.length <= 2) {
      return uniqueProducts.map(name => name!.substring(0, 15) + (name!.length > 15 ? '...' : '')).join(', ');
    } else {
      const firstTwo = uniqueProducts.slice(0, 2).map(name => name!.substring(0, 12) + (name!.length > 12 ? '...' : ''));
      return `${firstTwo.join(', ')} +${uniqueProducts.length - 2} more`;
    }
  };

  if (!isSignedIn || totalItems === 0 || !isCartPopupVisible || shouldHideOnCurrentPage) {
    return null;
  }
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">
                        {totalItems} item{totalItems !== 1 ? 's' : ''} • <PriceFormatter amount={getTotalPrice()} />
                      </p>
                    </div>
                  </div>
                  <Button
                    ...
                    {groupedItems.map(({ product, size, quantity }) => (
                      ...
                      >
                        ...
                        {product && product.images && (
                          <Link
                            href={'/product/' + (product.slug && product.slug.current ? product.slug.current : '')}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between mt-1">
                            <PriceFormatter
                              amount={product && product.price ? product.price * quantity : 0}
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => deleteCartProduct(product && product._id, size)}
                  ...
                  <div className="pt-4 border-t border-gray-200/30">
                    ...
                    <div className="flex gap-3">
                      <Link 
                        ...
                        onClick={() => setIsExpanded(false)}
                      >
                        <Button variant="outline" className="w-full text-sm border-gray-300 hover:bg-gray-50 rounded-xl transition-all duration-200">
                          View Cart
                        </Button>
                      </Link>
                      <Link 

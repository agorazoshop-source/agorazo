...
export default function ReelList({ reels, initialSlug }: ReelListProps) {
  ...
  const { addItem, addToFavorite, favoriteProduct, items, likedReels } =
    useStore();
  ...
  useEffect(() => {
    if (initialSlug && reels.length > 0) {
      const initialReelIndex = reels.findIndex(
        (reel) => reel.product.slug.current === initialSlug
      );
      if (initialReelIndex !== -1) {
        ...
        if (scrollContainerRef.current) {
          ...
          scrollContainerRef.current.scrollTop =
            initialReelIndex * containerHeight;
        }
      }
    }
  }, [initialSlug, reels]);
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-full">
      ...
      <div
        className={`w-full ${!isMobile && "md:w-[60%]"} h-full flex items-center justify-center -my-16`}
      >
        <div
              ...
              <div className="flex flex-col gap-4 h-1/2">
                ...
                <div>
                  <ProductImageCarousel
                    images={convertSanityImagesToUrls(
